...
object Log2parquet {
  def main(args: Array[String]): Unit = {
    ...
    val src: DataFrame = sparkSession.read.text(inputPath)
    val rdd: RDD[Row] = src.rdd.map(line => {
      val arr: Array[String] = line.getString(0).split(",", -1)
      Row(
        arr(0),
    ...
    val df  = sparkSession.createDataFrame(rdd,SchemaUtil.structType)
